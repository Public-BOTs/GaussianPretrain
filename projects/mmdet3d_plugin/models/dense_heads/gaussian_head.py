        ...
        for bs_idx in range(len(img_metas)):
            ...
            for cam_idx in range(self.cam_nums):
                ...
                render_img_rgb, render_img_depth = self.render(pts, 
                                                                ...
                                                                opacity_i, 
                                                                offset_i, 
                                                                img_metas[bs_idx]['cam_params'][cam_idx]['height'], 
                                                                img_metas[bs_idx]['cam_params'][cam_idx]['width'], 
                                                                fovx, fovy)
    ...
    def vis_grid(self, img_metas):
        ...
        for b in range(bs):
            for cam in range(len(img_metas[b]['lidar2img'])):
                lidar2img = torch.tensor(img_metas[b]['lidar2img'][cam][0], dtype=torch.float32)
                ...
                img = np.zeros(img_metas[b]['pad_before_shape'][cam])
                for point in voxel_grid_cam:
                    if point[2] > 0:
                        cv2.circle(img, (int(point[0]), int(point[1])), 2, (0, 255, 0), -1)
                cv2.imwrite(f'./vis/821.voxel_grid_bs.{b}_cam.{cam}.png', img)
